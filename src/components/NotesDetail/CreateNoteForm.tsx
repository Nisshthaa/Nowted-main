...
const { selectedFolder,setActiveNoteMode,setSelectedNoteId,setRefreshNotes} = useApp();
...
const handleCreate = async () => {
    ...
    try {
      ...
      await getNotes(selectedFolder.id)
    ...
    return (
        <div className="w-250 h-screen p-12.5 gap-15 flex flex-col ">
            ...
            />
                    ...
                    <button className=' flex justify-center items-center gap-3 w-65 h-10  bg-(--btn-bg) text-(--text-primary) cursor-pointer' onClick={handleCreate}
                       
                    ><Plus className='h-5 w-5'/> Add Note</button>
